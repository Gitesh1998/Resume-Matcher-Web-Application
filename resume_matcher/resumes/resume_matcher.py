...
def match_resume_to_job(job_desc, resumes_dir):
    ...
    for filename in os.listdir(resumes_dir):
        if filename.endswith('.pdf'):
            path = os.path.join(resumes_dir, filename)
            text = extract_text_from_pdf(path)  # Assumes this function is defined elsewhere
            resumes.append(text)
            filenames.append(filename)
    ...
    # Get indices of resumes sorted by similarity (high to low)
    ...
    for i in range(top_n):
        ...
        top_matches.append({
            ...
            'download_url': f"/media/resumes/{filenames[idx]}"
        })
